...
var mainState = {
	...
	create: function(){
		...
		this.ground = platforms.create(0, 500, 'ground');
		...
		this.player = game.add.sprite(game.width/8, 500, 'player');
		...
		this.obstacle = game.add.sprite(700,500, 'obstacle');
	...
	update: function(){
		...
		if (this.obstacle.x < 0) {
			this.obstacle.kill();
			this.obstacle = game.add.sprite(900,500, 'obstacle');
			this.obstacle.scale.setTo(1,0.2);
			this.obstacle.anchor.setTo(0,1);
			game.physics.arcade.enable(this.obstacle);
			this.obstacle.body.immovable = true;
